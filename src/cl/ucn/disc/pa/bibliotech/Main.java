...
public final class Main {
    ...
    private static void cambiarContrasenia(Sistema sistema) {
        StdOut.println("[*] Cambio de contraseña [*]");
        StdOut.println("Requisitos:");
        StdOut.println("""               
                - 5-16 cáracteres
                - Usar al menos una letra minúscula y una mayúscula
                - No usar espacios
                - Usar al menos una letra
                """);
        StdOut.println("Ingrese su nueva contraseña:");
        String contrasenia = StdIn.readString();

        Utils.validarContrasenia(contrasenia);

        sistema.sobreescribirContrasenia(contrasenia);
        StdIn.readLine();
    }

    private static void editarCorreo(Sistema sistema) {
        StdOut.println("[*] Editar correo [*]");
        StdOut.println("Ingrese su nueva direccion de correo electronico");
        String email = StdIn.readString();
        Utils.validarEmail(email);

        sistema.sobreescribirCorreo(email);
        StdIn.readLine();
    }

    /**
     * Inicia la calificacion de un libro, usando
     *
     * @param sistema a utilizar
     */
    private static void calificarLibro(Sistema sistema) {
        StdOut.println("[*] Calificar libro [*]");
        StdOut.println("Los libros disponibles para calificar son:");
        StdOut.println("");

        StdOut.println(sistema.obtegerCatalogoLibros());

        StdOut.println("[*] Calificar [*]");
        StdOut.println("Ingrese el ISBN del libro a calificar");
        String inputIsbn = StdIn.readString();

        StdOut.println("Ingrese la calificacion que desea darle al libro: (0 - 5)");
        int inputCalificacion = StdIn.readInt();

        //Se valida que el libro exista y luego que si se califico correctamente
        if (inputCalificacion < 0 || inputCalificacion > 5) {
            throw new IllegalArgumentException("Ingreso una calificacion invalida!");
        }

        sistema.actualizarCalificacion(inputIsbn, inputCalificacion);

        StdOut.println("Califico con " + inputCalificacion + " estrellas al libro");
        StdOut.println("");
        StdIn.readLine();
    }
}
